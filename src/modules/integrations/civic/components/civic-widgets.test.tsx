...
type Case = { name: string; widget: Widget; method: keyof typeof provider; data: unknown[] };

const CASES: Case[] = [
    { name: "ServiceFinder", widget: ServiceFinder as Widget, method: "getServiceDetails", data: musterstadtServiceDetails },
    { name: "OpeningHours", widget: OpeningHours as Widget, method: "getOpeningHours", data: musterstadtOpeningHours },
    { name: "DepartmentDirectory", widget: DepartmentDirectory as Widget, method: "getDepartments", data: musterstadtDepartments },
    { name: "CouncilBlock", widget: CouncilBlock as Widget, method: "getCouncilBodies", data: musterstadtCouncilBodies },
    { name: "EventsGrid", widget: EventsGrid as Widget, method: "getEvents", data: musterstadtEvents },
    { name: "ServiceGrid", widget: ServiceGrid as Widget, method: "getServices", data: musterstadtServices },
    { name: "NewsGrid", widget: NewsGrid as Widget, method: "getNews", data: musterstadtNews },
    { name: "WasteCalendar", widget: WasteCalendar as Widget, method: "getWasteCollectionEntries", data: musterstadtWasteEntries },
    { name: "ContactCard", widget: ContactCard as Widget, method: "getContacts", data: musterstadtContacts },
];
...
describe.each(CASES)("$name", ({ name, widget, method, data }) => {
    ...
    it("renders its content, with no fallback state, when data is present", async () => {
        provider[method].mockResolvedValue(ok(data));
        const { container } = await renderWidget(widget);
...
describe("NewsAndEventsSplit (two feeds)", () => {
    ...
    it("keeps rendering the feed that loaded when the other one fails (graceful degradation)", async () => {
        provider.getNews.mockResolvedValue(failure());
        provider.getEvents.mockResolvedValue(ok(musterstadtEvents));
        const { container } = await renderWidget(NewsAndEventsSplit as Widget);
        expect(container.querySelector("[data-widget-state]")).toBeNull();
        expect(container.textContent).toContain(musterstadtEvents[0].title);
        expect(logError).toHaveBeenCalledTimes(1);
    });
...
describe("AlertBanner (deliberately different rules)", () => {
    ...
    it("renders its alerts normally when data is present", async () => {
        provider.getAlerts.mockResolvedValue(ok(musterstadtAlerts));
        const { container } = await renderWidget(AlertBanner as Widget);
        expect(container.querySelector("[data-widget-state]")).toBeNull();
